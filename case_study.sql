...
select a.customer_id,sum(b.price) prices 
from sales a,menu b where 
a.product_id=b.product_id group by a.customer_id; 
...
select distinct(customer_id),min(order_date),product_id
...
select customer_id,product_id,var_count from
(select customer_id,product_id,var_count,
rank() over( partition by customer_id order by  var_count desc) as var_rank from
(select customer_id, product_id,count(product_id) as var_count from 
...
select customer_id,product_id,var_rank from (
select s.customer_id,s.product_id,
rank() over(partition by s.customer_id order by s.order_date) as var_rank from 
sales s
join
members m
...
(select var2_join.customer_id,var2_join.total_amount_spent,
RANK() OVER(partition by var2_join.customer_id order by var2_join.price) as rowrank from
( select var_join.customer_id,var_join.total_item,mm.price,
